...
```{r eval=T, fig.width=7, fig.height=3.6}
...
Here, we try to exclude transitions after reference bias is accounted for, to evalutate whether PCA results are affected by DNA degradataion
...
Including transitions in PCA does not lead to observable batch effects, and excluding transition does not change the results significantly, suggesting the DNA degradation is not an major factor causing batch effects in PCA.
...
```{r eval=TRUE, fig.width=7, fig.height=3.9}
rename_pop <- tibble(population = c("ITV2011", "KNG2011", "QQL2011", "BUK2011", "IKE2011", "PAA2011", "ATP2011", "NAR2008", "UUM2010"),
                     population_new =c("pop 1", "pop 2", "pop 3", "pop 4", "pop 5", "pop 6", "pop 7", "pop 8", "pop 9"))
set.seed(42)
p_c <- het_final %>%
...
```{r eval=TRUE,fig.width=7, fig.height=3.6}
...
```{r eval=TRUE, fig.width=7, fig.height=17.6}
