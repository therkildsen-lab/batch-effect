...
```{bash eval=FALSE}
## A pair of fastq files for trimming
...
## IKE before and after sliding window trimming
## Come up with a bam list
rm /workdir/batch-effect/tutorial/data/bam_list_poly_g.txt

for LINE in `egrep 'IKE' /workdir/batch-effect/sample_lists/bam_list_realigned.txt`; do
  TEMP=${LINE%.*}
  PREFIX=${TEMP/*\/}
  echo /workdir/batch-effect/tutorial/data/${PREFIX}_subsetted_poly_g_trimming.bam >> /workdir/batch-effect/tutorial/data/bam_list_poly_g.txt
  echo /workdir/batch-effect/tutorial/data/${PREFIX}_subsetted_sliding_window_trimming.bam >> /workdir/batch-effect/tutorial/data/bam_list_poly_g.txt
done

## Come up with a sample table
head -n 1 /workdir/batch-effect/sample_lists/sample_table_merged.tsv >  /workdir/batch-effect/tutorial/data/sample_table_poly_g.tsv
egrep 'IKE' /workdir/batch-effect/sample_lists/sample_table_merged.tsv >>  /workdir/batch-effect/tutorial/data/sample_table_poly_g.tsv

## Subset bam files
for LINE in `egrep 'IKE' /workdir/batch-effect/sample_lists/bam_list_realigned.txt`; do
  TEMP=${LINE%.*}
  PREFIX=${TEMP/*\/}
  samtools view -b $LINE "LG06:5000000-15000000" > /workdir/batch-effect/tutorial/data/${PREFIX}_subsetted_sliding_window_trimming.bam
  PREFIX1=${PREFIX%sorted*}
  PREFIX2=${PREFIX/*gadMor3}
  samtools view -b /workdir/cod/greenland-cod/bam/${PREFIX1}minq20${PREFIX2}.bam "LG06:5000000-15000000" > /workdir/batch-effect/tutorial/data/${PREFIX}_subsetted_poly_g_trimming.bam
done
```
...
```{bash eval=FALSE}
## Come up with a bam list
rm /workdir/batch-effect/tutorial/data/bam_list_bq.txt

for LINE in `egrep 'IKE' /workdir/batch-effect/sample_lists/bam_list_realigned.txt`; do
  TEMP=${LINE%.*}
  PREFIX=${TEMP/*\/}
  echo /workdir/batch-effect/tutorial/data/${PREFIX}_subsetted_bq.bam >> /workdir/batch-effect/tutorial/data/bam_list_bq.txt
done
